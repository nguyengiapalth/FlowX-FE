        ...
        <div className="text-center py-12">
          ...
          <button
            onClick={() => navigate('/departments')}
                            className="px-4 py-2 btn-gradient rounded-lg transition-colors"
          >
      ...
      <nav className="flex mb-4" aria-label="Breadcrumb">
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          <li className="inline-flex items-center">
            <Link to="/departments" className="inline-flex items-center text-sm font-medium text-gray-700 hover:text-primary-600">
                ...
                {members.map((member) => (
                  <div key={member.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => handleViewProfile(member.id)}
                        className="text-primary-600 hover:text-primary-900 text-sm font-medium ml-4 whitespace-nowrap"
                      >
                        Xem profile
                      </button>
                    </div>
            ...
            {projects.length === 0 ? (
              <div className="text-center py-12">
                ...
                {(isDepartmentManager(departmentIdNum) || isGlobalManager()) && (
                  <button
                    onClick={() => setShowCreateProjectModal(true)}
                    className="inline-flex items-center space-x-2 px-6 py-3 btn-gradient rounded-lg transition-colors font-medium"
                  >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                          project.status === 'IN_PROGRESS' ? 'bg-primary-100 text-primary-800' :
                                          project.status === 'COMPLETED' ? 'bg-secondary-100 text-secondary-800' :
              ...
              <div className="flex space-x-3">
                ...
                {isDepartmentManager(departmentIdNum) && !isEditingInfo && !isEditingManager && (
                  <button
                    ...
                    className="flex items-center space-x-2 px-4 py-2 btn-gradient rounded-lg transition-colors"
                  >
                    <Edit className="w-4 h-4" />
