...
class GoogleAuthService {
   ...
   */
  async signInWithPopup(): Promise<{ idToken: string; accessToken: string; user: GoogleUser }> {
    await this.initializeGoogleAuth();
    ...
    return new Promise((resolve, reject) => {
      // Initialize OAuth2 token client for getting access token
   ...
   */
  async signInWithOneTap(): Promise<{ idToken: string; user: GoogleUser }> {
    await this.initializeGoogleAuth();
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async (response: any) => {
          try {
            ...
            const user = await this.parseJWT(idToken);
   ...
   */
  private async parseJWT(idToken: string): Promise<GoogleUser> {
    try {
